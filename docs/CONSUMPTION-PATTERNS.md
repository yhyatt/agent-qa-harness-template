...
| Do you want to pull upstream harness improvements via `git pull upstream HEAD`? | C. Fork via GitHub UI |
...
`git pull upstream HEAD` uses the upstream's default branch automatically; works whether the template's default is master, main, or anything else.
