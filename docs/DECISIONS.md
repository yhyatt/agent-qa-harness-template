...
1. Move final report generation out of `test.afterAll` in `journeys.spec.ts` and into a Playwright `globalTeardown` (`tests/e2e/global-teardown.ts`) that runs once, after every project finishes. Each project's `afterAll` now calls `writeProjectSidecar`, which stamps `project` onto its `JourneyResult`s and `StepFinding`s and writes them to a per-project JSON sidecar under a gitignored `.qa-runs/<run>/.partials/` directory. The teardown calls `aggregateRunReport`, which reads every sidecar, concatenates results and findings, and writes the combined `findings.json` and `REPORT.md`. A companion `globalSetup` (`tests/e2e/global-setup.ts`) clears stale sidecars from `.partials/` at run start, so a rerun that reuses the run dir (the default timestamp is minute-granular, and `QA_RUN_DIR` can be pinned) does not resurrect a project that the rerun excluded: a narrower `test:e2e:desktop` rerun after a full both-projects run would otherwise merge mobile's stale sidecar. The project list for the combined report is the union of every sidecar's declared project plus any project seen in results or axe surfaces, so a project that ran but produced zero results still gets a section. Project names are sanitized to a safe path segment (dot-only and empty names collapse to a literal fallback) before use in sidecar filenames and screenshot directories, so a project named `.` or `..` cannot escape the run directory.
